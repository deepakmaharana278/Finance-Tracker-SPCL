...
const StatCard = ({ label, value, color, icon, trend }) => (
  <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 hover:scale-105 transition-transform duration-300">
    <div className="flex items-start justify-between mb-3">
      ...
      <div
        className={`w-8 h-8 rounded-xl ${color === "text" ? "bg-gray-100 dark:bg-gray-700" : color === "accent" ? "bg-green-100 dark:bg-green-900/30" : "bg-red-100 dark:bg-red-900/30"} flex items-center justify-center`}
      >
    ...
    <div className="flex items-baseline gap-2">
      ...
      {trend && <span className="text-xs font-medium text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 px-1.5 py-0.5 rounded-full">{trend}</span>}
    </div>
  </div>
);
          ...
          {data.map((d) => (
            <div key={d.month} className="flex flex-col items-center gap-2 flex-1">
              <div className="flex items-end gap-1 w-full h-36">
                <div className="flex-1 rounded-t-lg transition-all duration-500 hover:opacity-100 bg-green-500" style={{ height: `${Math.max((d.income / max) * 126, 4)}px`, opacity: 0.85 }} />

                <div className="flex-1 rounded-t-lg transition-all duration-500 hover:opacity-100 bg-red-500" style={{ height: `${Math.max((d.expense / max) * 126, 4)}px`, opacity: 0.85 }} />
              </div>
              <span className="text-xs font-medium text-gray-600 dark:text-gray-400">{d.month.slice(5)}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between items-center text-sm">
        ...
        <span className="font-semibold text-red-600 dark:text-red-400">₹{Number(total).toLocaleString("en-IN")}</span>
      </div>
      <div className="relative w-full h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
        <div className="absolute left-0 top-0 h-full bg-linear-to-r from-red-400 to-red-600 rounded-full transition-all duration-500" style={{ width: `${percentage}%` }} />
      </div>
    </div>
  );
};
...
export default function Dashboard() {
  ...
  const maxCategoryTotal = summary?.top_categories?.length ? Math.max(...summary.top_categories.map((c) => Number(c.total))) : 0;
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2 text-sm">Your financial health at a glance</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <StatCard label="Total Balance" value={summary?.balance || 0} color="text" icon="fas fa-wallet" trend={summary?.balance > 0 ? "+12%" : null} />
          <StatCard label="Total Income" value={summary?.income || 0} color="accent" icon="fas fa-arrow-trend-up" trend="+8%" />
          <StatCard label="Total Expenses" value={summary?.expense || 0} color="danger" icon="fas fa-arrow-trend-down" trend="-3%" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
              ...
              <div className="space-y-4">
                {summary.top_categories.map((c) => (
                  <CategoryProgress key={c.category__name || "uncategorised"} name={c.category__name ?? "Uncategorised"} total={Number(c.total)} maxTotal={maxCategoryTotal} />
                ))}
              </div>
            </div>
          )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => navigate("/transactions")} className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium">
                View All →
              </button>
            </div>
            ...
            {recent.slice(0, 5).length > 0 ? (
                  ...
                  <div key={t.id} className="py-3 first:pt-0 last:pb-0">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 flex-1">
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${t.type === "income" ? "bg-green-100 dark:bg-green-900/30" : "bg-red-100 dark:bg-red-900/30"}`}
                        >
              ...
              <div className="text-center py-8">
                ...
                <button onClick={() => navigate("/add")} className="mt-4 text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 font-medium">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-blue-100 text-sm">
                    {summary.expense > summary.income ? "⚠️ Your expenses exceed your income. Consider reviewing your spending." : "🎉 Great job! You're maintaining a healthy financial balance."}
                  </p>
                </div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold">{(((summary.income - summary.expense) / (summary.income || 1)) * 100).toFixed(1)}%</div>
